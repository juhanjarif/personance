  ...
  const checkForGoalMet = async () => {
    try {
      ...
      for (const goal of goalsRes.data) {
        ...
        const goalTxs = txRes.data.filter(
          (t) => new Date(t.created_at) >= goalCreated,
        );
          ...
          >
            <div className="md:col-span-2 flex p-1 bg-gray-100 dark:bg-gray-700 rounded-lg">
              <button
                type="button"
                onClick={() =>
                  setFormData({ ...formData, type: "expense", typeId: 2 })
                }
              ...
              <button
                type="button"
                onClick={() =>
                  setFormData({ ...formData, type: "income", typeId: 1 })
                }
              ...
              <button
                type="button"
                onClick={() =>
                  setFormData({ ...formData, type: "transfer", typeId: 3 })
                }
            ...
            {formData.type === "transfer" ? (
              <div className="space-y-1">
                ...
                >
                  <option value="">Select Account</option>
                  {accounts
                    .filter((a) => String(a.account_id) !== formData.accountId)
                    .map((a) => (
                      <option key={a.account_id} value={a.account_id}>
                        {a.account_name}
                      </option>
              ...
              {transactions.map((t) => (
                ...
                >
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                    {new Date(
                      t.created_at.includes("Z") || t.created_at.includes("+")
                        ? t.created_at
                        : t.created_at + "Z",
                    )
                  ...
                  <td
                    className={`px-6 py-4 text-sm text-right font-black whitespace-nowrap ${t.transaction_type === "income" ? "text-emerald-500" : t.transaction_type === "expense" ? "text-red-500" : "text-blue-500"}`}
                  >
