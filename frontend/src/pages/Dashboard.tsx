...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [accRes, txRes, budgetRes, goalsRes, loansRes] =
          await Promise.all([
        ...
        const totalBalance = accRes.data.reduce(
          (sum, acc) => sum + parseFloat(acc.current_balance),
          0,
        );
        ...
        if (totalBudget) {
          ...
          setSpentThisMonth(
            relevantTxs.reduce((sum, t) => sum + parseFloat(t.amount), 0),
          );
        }
      } catch (err) {
        console.error(err);
  ...
  const goalProgressBalance = currentGoal
    ? parseFloat(currentGoal.current_amount || "0")
    : 0;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <div className="lg:col-span-8 space-y-8">
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Link to="/accounts" className="block group">
            <div className="p-8 rounded-3xl bg-linear-to-br from-blue-600 to-indigo-700 text-white shadow-2xl shadow-blue-500/20 transform group-hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="grid grid-cols-1 gap-3">
            <Link
              ...
              className="py-3 px-6 rounded-2xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-bold border border-gray-100 dark:border-gray-700 hover:border-blue-500 transition-all flex justify-between items-center group shadow-sm"
            >
            ...
            <Link
              ...
              className="py-3 px-6 rounded-2xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-bold border border-gray-100 dark:border-gray-700 hover:border-emerald-500 transition-all flex justify-between items-center group shadow-sm"
            >
            ...
            <Link
              ...
              className="py-3 px-6 rounded-2xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-bold border border-gray-100 dark:border-gray-700 hover:border-amber-500 transition-all flex justify-between items-center group shadow-sm"
            >
                  ...
                  {recentTransactions.map((t) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        <span
                          className={`text-sm font-black ${t.transaction_type === "income" ? "text-emerald-500" : "text-red-500"}`}
                        >
                          {t.transaction_type === "income" ? "+" : "-"} Tk.{" "}
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="p-8 rounded-3xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-200/50 dark:shadow-none">
          ...
          {budget ? (
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                <p className="text-3xl font-black text-gray-900 dark:text-white">
                  Tk. {spentThisMonth.toLocaleString()}
                </p>
                <span
                  className={`text-xs font-black uppercase ${spentThisMonth > parseFloat(budget.amount_limit) ? "text-red-500" : "text-blue-500"}`}
                >
                  {Math.round(
                    (spentThisMonth / parseFloat(budget.amount_limit)) * 100,
                  )}
                  %
                </span>
              </div>
              <div className="h-3 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (spentThisMonth / parseFloat(budget.amount_limit)) * 100)}%`,
                  }}
                ></div>
              </div>
              <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">
                Spent of Tk. {Number(budget.amount_limit).toLocaleString()}
              </p>
            </div>
          ) : (
            <div className="text-center py-6">
              ...
              <Link
                to="/planning?tab=budget"
                className="inline-block px-6 py-2 rounded-xl bg-blue-50 text-blue-600 font-bold text-xs"
              >
                Add Budget &rarr;
              </Link>
            </div>
        ...
        <div className="p-8 rounded-3xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-700/50 shadow-sm relative overflow-hidden min-h-62.5 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-6">
              ...
              {goals.length > 1 && (
                <div className="flex space-x-2">
                  <button
                    onClick={prevGoal}
                    className="p-1 px-2.5 rounded-lg bg-emerald-100 dark:bg-emerald-800 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-200 transition-all font-bold text-sm shadow-sm hover:scale-105 active:scale-95"
                  >
                    &larr;
                  </button>
                  <button
            ...
            {goals.length > 0 ? (
              <Link
                key={currentGoal?.financial_goal_id}
                to="/planning?tab=goal"
                className="fade-in block group"
              >
                <div className="space-y-4">
                      ...
                      {currentGoal.deadline && (
                        <p className="text-xs font-medium uppercase text-emerald-100/90 tracking-wide mt-auto">
                          ...
                          {Math.round(
                            Math.max(
                              0,
                              (parseFloat(currentGoal.target_amount) -
                                parseFloat(currentGoal.current_amount || "0")) /
                                Math.max(
                                  1,
                                  (new Date(
                                    ...
                                    12 +
                                    (new Date(currentGoal.deadline).getMonth() -
                                      new Date().getMonth()),
                                ),
                            ),
                          ).toLocaleString()}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="h-2.5 w-full bg-emerald-100 dark:bg-emerald-800 rounded-full overflow-hidden shadow-inner">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, (goalProgressBalance / parseFloat(currentGoal.target_amount)) * 100))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="text-center py-10">
                <p className="text-emerald-400 text-sm italic font-medium mb-4">
                  No goals active
                </p>
                <Link
                  to="/planning?tab=goal"
                  className="inline-block px-6 py-2 rounded-xl bg-emerald-100/50 text-emerald-700 font-bold text-xs"
                >
                  Add Goal &rarr;
                </Link>
              </div>
          ...
          {goals.length > 0 && (
            <div className="mt-6 flex justify-center space-x-1.5">
              {goals.map((_, i) => (
                <div
                  key={i}
                  className={`h-1.5 rounded-full transition-all duration-300 ${i === currentGoalIndex ? "w-6 bg-emerald-500" : "w-1.5 bg-emerald-200"}`}
                ></div>
              ))}
            </div>
          )}
        ...
        <div className="p-8 rounded-3xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-700/50 shadow-sm relative overflow-hidden min-h-62.5 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-sm font-black uppercase tracking-[0.2em] text-amber-600 dark:text-amber-400">
                Active Loans
              </h3>
              {loans.length > 1 && (
                <div className="flex space-x-2">
                  <button
                    onClick={prevLoan}
                    className="p-1 px-2.5 rounded-lg bg-amber-100 dark:bg-amber-800 text-amber-600 dark:text-amber-400 hover:bg-amber-200 transition-all font-bold text-sm shadow-sm hover:scale-105 active:scale-95"
                  >
                    &larr;
                  </button>
                  <button
            ...
            {loans.length === 0 ? (
              <div className="text-center py-6">
                <p className="text-amber-400 text-sm italic font-medium mb-4">
                  No active loans
                </p>
                <Link
                  to="/loans"
                  className="inline-block px-6 py-2 rounded-xl bg-amber-100/50 text-amber-900 dark:text-amber-300 font-bold text-xs hover:bg-amber-100 transition-colors"
                >
                  Add Loan &rarr;
                </Link>
              </div>
                    ...
                    <div className="flex items-baseline flex-wrap">
                      ...
                      <span className="text-xl font-black text-gray-900 dark:text-white mr-2">
                        ...
                        {Math.max(
                          0,
                          parseFloat(
                            currentLoan.total_repayment_amount ||
                              currentLoan.principal_amount,
                          ) - parseFloat(currentLoan.paid_amount || "0"),
                        ).toLocaleString()}
                      </span>
                      ...
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-sm font-black text-amber-600 dark:text-amber-400">
                          ...
                          {(() => {
                            ...
                            let freq = 1;
                            const rf = (
                              currentLoan.return_frequency || "MONTHLY"
                            ).toUpperCase();
                      ...
                      <div className="flex justify-between items-center text-[10px] font-bold text-gray-500">
                        ...
                        <span>
                          {(() => {
                            ...
                            const graceDays =
                              (currentLoan.grace_period_months || 0) * 30;
                            let freqDays = 30;
                            const rf = (
                              currentLoan.return_frequency || "MONTHLY"
                            ).toUpperCase();
                            ...
                            const instSize = (() => {
                              ...
                              const em = Math.max(
                                1,
                                tm - (currentLoan.grace_period_months || 0),
                              );
                              let f = 1;
                              if (rf === "QUARTERLY") f = 3;
                            ...
                            nextDate.setDate(
                              nextDate.getDate() +
                                graceDays +
                                (installmentsPaid + 1) * freqDays,
                            );

                            const finalDue = new Date(
                              currentLoan.due_date || nextDate,
                            );
                            return (
                              nextDate > finalDue ? finalDue : nextDate
                            ).toLocaleDateString();
                          })()}
                        </span>
                      </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="h-2.5 w-full bg-amber-100 dark:bg-amber-800 rounded-full overflow-hidden shadow-inner">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, (parseFloat(currentLoan.paid_amount || "0") / parseFloat(currentLoan.total_repayment_amount || currentLoan.principal_amount)) * 100))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
          ...
          {loans.length > 0 && (
            <div className="mt-6 flex justify-center space-x-1.5">
              {loans.map((_, i) => (
                <div
                  key={i}
                  className={`h-1.5 rounded-full transition-all duration-300 ${i === currentLoanIndex ? "w-6 bg-amber-500" : "w-1.5 bg-amber-200"}`}
                ></div>
              ))}
            </div>
          )}
