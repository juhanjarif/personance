...
const Planning: FC = () => {
  ...
  const handleGoalSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (
      new Date(goalForm.deadline) < new Date(new Date().setHours(0, 0, 0, 0))
    ) {
    ...
    try {
      ...
      console.error(err);
      alert(
        "Failed to add goal: " + (err.response?.data?.message || err.message),
      );
    }
  };
  ...
  return (
    <div className="space-y-12">
      <div className="flex space-x-4 border-b border-gray-100 dark:border-gray-700">
        {(["budget", "goal"] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => navigate(`/planning?tab=${tab}`, { replace: true })}
      ...
      <section className="space-y-6">
        ...
        {activeTab === "budget" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-4">
              <div className="card">
                <h4 className="text-sm font-black uppercase tracking-widest text-gray-400 mb-6">
                  Setup Budget
                </h4>
                <form onSubmit={handleBudgetSubmit} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20"
                  >
            ...
            <div className="lg:col-span-8">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-8 text-white shadow-2xl">
                <p className="text-blue-100 text-xs font-black uppercase tracking-[0.2em] mb-4">
                  Active Plan
                </p>
                {totalBudget ? (
                    ...
                    <div>
                      ...
                      <div className="mt-4 space-y-1">
                        <p className="text-blue-200 text-[10px] font-bold uppercase">
                          ...
                          &mdash;{" "}
                          {new Date(totalBudget.end_date).toLocaleDateString()}
                        </p>
                        <p className="text-blue-300 text-[10px] italic opacity-60">
                          Created on{" "}
                          {new Date(totalBudget.created_at).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    <button
        ...
        {activeTab === "goal" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-4">
              <div className="card">
                <h4 className="text-sm font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400 mb-6">
                  New Saving Goal
                </h4>
                <form onSubmit={handleGoalSubmit} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20"
                  >
                    Create Goal
                  </button>
                </form>
            ...
            <div className="lg:col-span-8">
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-xl">
                <h4 className="text-[15px] font-black uppercase tracking-[0.2em] text-gray-400 mb-8">
                ...
                {goals.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {goals.map((goal) => (
                      <div
                        key={goal.financial_goal_id}
                        className="p-6 rounded-2xl bg-emerald-50/50 dark:bg-emerald-900/5 border border-emerald-100 dark:border-emerald-800/50 relative group"
                      >
                        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-2">
                          ...
                          <button
                            onClick={() => deleteGoal(goal.financial_goal_id)}
                            className="bg-red-50 text-red-400 hover:bg-red-100 hover:text-red-500 p-1.5 rounded-lg transition-colors"
                          >
                            <svg
                        ...
                        <p className="text-[22px] font-black text-emerald-600 dark:text-white uppercase tracking-tight mb-4 shadow-sm">
                          {goal.goal_name}
                        </p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden transform transition-all scale-100">
            ...
            <div className="p-6 space-y-6">
              <div className="flex justify-between items-center p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/30">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-400">
                    Tk. {Number(selectedGoal.target_amount).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleContribution} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-emerald-500 text-white font-black uppercase tracking-wider hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20"
                >
