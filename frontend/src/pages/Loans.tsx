...
const Loans = () => {
  ...
  const [form, setForm] = useState({
    ...
    returnFrequency: "MONTHLY" as any,
    gracePeriodMonths: 0,
  });
  ...
  const calculateLoan = () => {
    ...
    if (P <= 0 || !form.interestType || !form.returnFrequency) {
    ...
    const diffMonths = Math.max(
      1,
      Math.ceil(diffTime / (1000 * 60 * 60 * 24 * 30.44)),
    );
    ...
    if (form.interestType === "simple") {
      ...
      if (nMonths > 0 && rPerMonth > 0) {
        const emi =
          (P * rPerMonth * Math.pow(1 + rPerMonth, nMonths)) /
          (Math.pow(1 + rPerMonth, nMonths) - 1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const accrualStartDate = new Date();
      accrualStartDate.setMonth(
        accrualStartDate.getMonth() + (form.gracePeriodMonths || 0),
      );
      ...
      const payload = {
        ...form,
        paymentFrequency: form.returnFrequency.toLowerCase(),
  ...
  const toggleStatus = async () => {
    ...
    try {
      await api.patch(`/loans/${selectedLoan.loan_id}/status`, {
        status: newStatus,
      });
      fetchLoans();
        ...
        <div className="lg:col-span-4">
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-amber-100 dark:border-amber-900/20 shadow-xl relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, interestType: e.target.value as any })
                    }
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    required
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        returnFrequency: e.target.value as any,
                      })
                    }
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    required
              ...
              <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-lg font-black text-amber-600 dark:text-amber-400">
                    Tk.{" "}
                    {calculations.totalRepayment.toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-amber-700/60 dark:text-amber-300/60 uppercase">
                    Next Installment Amount
                  </span>
                  <span className="text-xs font-bold text-amber-600/80 dark:text-amber-400/80">
                    Tk.{" "}
                    {calculations.nextInstallment.toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </span>
                </div>
              ...
              <button
                type="submit"
                className="w-full py-3.5 rounded-xl bg-amber-400 hover:bg-amber-500 text-amber-900 font-black uppercase tracking-wider shadow-lg shadow-amber-400/20 transition-all transform hover:-translate-y-0.5"
              >
                Add Loan
              </button>
            </form>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredLoans.map((loan) => (
                <div
                  ...
                  className={`p-6 rounded-2xl border transition-all relative group cursor-pointer hover:-translate-y-1 ${
                    loan.status === "active"
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <span
                        className={`inline-block px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest mb-2 ${loan.status === "active" ? "bg-emerald-200 text-emerald-800" : "bg-red-200 text-red-800"}`}
                      >
                        {loan.status}
                      </span>
                    ...
                    <div className="flex items-baseline flex-wrap">
                      ...
                      <span className="text-2xl font-black text-gray-900 dark:text-white mr-2">
                        ...
                        {Math.max(
                          ...
                          ) - parseFloat(loan.paid_amount || "0"),
                        ).toLocaleString()}
                      </span>
                      <span className="text-xs font-bold text-gray-400">
                        of Tk.{" "}
                        {Number(
                          loan.total_repayment_amount || loan.principal_amount,
                        ).toLocaleString()}
                      </span>
                    </div>
                        ...
                        <span className="text-sm font-black text-amber-600 dark:text-amber-400">
                          ...
                          {(() => {
                            ...
                            const start = new Date(
                              loan.created_at || loan.start_date,
                            );
                            const due = new Date(
                              loan.due_date ||
                                new Date(
                        ...
                        <span>
                          {(() => {
                            ...
                            nextDate.setDate(
                              nextDate.getDate() +
                                graceDays +
                                nextInstIndex * freqDays,
                            );

                            const finalDue = new Date(
                              loan.due_date || nextDate,
                            );
                            return (
                    ...
                    <div className="mt-4">
                      ...
                      <div className="h-2.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, Math.max(0, (parseFloat(loan.paid_amount || "0") / parseFloat(loan.total_repayment_amount || loan.principal_amount)) * 100))}%`,
                          }}
                        ></div>
                      </div>
                    </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden transform transition-all scale-100">
            ...
            <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-amber-600 dark:text-amber-400">
                    ...
                    {Math.max(
                      0,
                      parseFloat(
                        selectedLoan.total_repayment_amount ||
                          selectedLoan.principal_amount,
                      ) - parseFloat(selectedLoan.paid_amount || "0"),
                    ).toLocaleString()}
                  </p>
                </div>
              ...
              <form onSubmit={handleRepayment} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-amber-400 text-amber-900 font-black uppercase tracking-wider hover:bg-amber-500 transition-colors shadow-lg shadow-amber-400/20"
                >
                  Process Payment
                </button>
              </form>
