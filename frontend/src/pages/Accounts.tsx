            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    accountTypeId: parseInt(e.target.value),
                  })
                }
                required
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map((acc) => {
          ...
          return (
            <div
              key={acc.account_id}
              className={`card flex flex-col justify-between h-48 border-l-4 ${borderClass} hover:shadow-lg transition-shadow bg-white dark:bg-gray-800`}
            >
              <div>
                <div className="flex justify-between items-start mb-4">
                  {editingAccount?.account_id === acc.account_id ? (
                    ...
                    <>
                      <h3 className="font-bold text-gray-900 dark:text-white text-lg truncate flex-1">
                        {acc.account_name}
                      </h3>
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
                          {acc.type_name}
                        </span>
                        <button
              ...
              <p className="text-3xl font-black text-gray-900 dark:text-white">
                <span className={`text-lg font-bold mr-1 ${textClass}`}>
                  Tk.
                </span>
