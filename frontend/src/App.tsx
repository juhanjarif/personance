...
const Layout: FC<LayoutProps> = ({ children }) => {
  ...
  const linkClass = (path: string, search?: string) => {
    ...
    return `relative py-2 text-sm font-bold transition-all ${active
  ...
  const indicatorClass = (path: string, search?: string) => {
    return `absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 transition-all duration-300 transform ${isActive(path, search) ? 'scale-x-100' : 'scale-x-0'
      }`;
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="md:hidden flex justify-center py-2 border-t border-gray-100 dark:border-gray-700 space-x-4">
          ...
          <Link to="/loans" className="text-[10px] font-black uppercase text-gray-500 dark:text-gray-400 hover:text-blue-600">Loans</Link>
        </div>
      </nav>
